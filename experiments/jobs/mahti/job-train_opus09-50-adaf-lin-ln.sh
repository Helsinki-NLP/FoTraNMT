#!/bin/bash
#SBATCH --job-name=train_opus09-50-adaf-lin-ln
...
export EXP_ID=opus09-50-adaf-lin-ln
...
srun singularity_wrapper exec --nv $SAND_BOX \
      python -u ${ONMT}/train.py \
            -config ${EXP_DIR}/config/config-opus09-50-adaf.yml \
            -save_model /scratch/project_2005099/models/opus09/${EXP_ID} \
            -ab_layers lin \
            -ab_fixed_length 50 \
            -hidden_ab_size 2048 \
