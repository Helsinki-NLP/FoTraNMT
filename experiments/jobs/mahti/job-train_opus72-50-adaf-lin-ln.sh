#!/bin/bash
#SBATCH --job-name=train_opus72-50-adaf-lin-ln
...
export EXP_ID=opus72-50-adaf-lin-ln

srun bash python_multinode_wrapper.sh ${ONMT}/train.py \
            -config ${EXP_DIR}/config/config-opus72-50-adaf.yml \
            -save_model /scratch/project_2005099/models/opus72/${EXP_ID} \
            -ab_layers lin \
            -ab_fixed_length 50 \
            -hidden_ab_size 2048 \
            -ab_layer_norm layernorm \
            -enc_layers 5 \
            -train_steps 100000 \
